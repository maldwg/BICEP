...
async def test_start_static_container_analysis_from_idle_container(
    ...
    static_analysis_mock_data: StaticAnalysisData = StaticAnalysisData(
        container_id=1, ensemble_id=None, dataset_id=1
    )
    ...
    response = await start_static_container_analysis(
        static_analysis_data=static_analysis_mock_data, db=db_session
    )
    response_json = json.loads(response.body.decode())
    print(response_json)
...
async def test_start_static_container_analysis_from_idle_container_unsuccesfully(
    ...
    assert response_json == {
        "error": "container 1 - static analysis could not be triggered"
    }
...
async def test_start_network_container_analysis_from_idle_container(
    ...
    response = await start_network_container_analysis(
        network_analysis_data=network_analysis_mock_data, db=db_session
    )
    response_json = json.loads(response.body.decode())
    assert response.status_code == 200
...
async def test_start_network_container_analysis_from_idle_container_unsuccesfully(
    ...
    assert response.status_code == 500
    assert response_json == {
        "error": "container 1 - network analysis could not be triggered"
    }
...
@pytest.mark.asyncio
async def test_stop_analysis_for_ensemble_container_unsucsessfully(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    ...
    print(response_json)
    assert response_json == {
        "error": f"Container is part of a running ensemble. It is not possible to stop a container analysis individually"
    }
...
async def test_stop_analysis_successfully(db_session_fixture: DatabaseSessionFixture):
    ...
    assert response.status_code == 200
    assert response_json == {
        "message": f"Analysis for container {stop_analysis_data.container_id} stopped successfully"
    }
...
async def test_stop_analysis_unsuccessfully(db_session_fixture: DatabaseSessionFixture):
    ...
    assert response.status_code == 500
    assert response_json == {
        "error": f"Analysis for container {stop_analysis_data.container_id} did not stop successfully"
    }
...
async def test_finished_analysis(db_session_fixture: DatabaseSessionFixture):
    db_session = await db_session_fixture.get_db_session()
    analysis_finished_data: AnalysisFinishedData = AnalysisFinishedData(container_id=1)

    mock_ids_container = await db_session_fixture.get_ids_container_model()

    response = await finished_analysis(
        analysisFinishedData=analysis_finished_data, db=db_session
    )
    response_json = json.loads(response.body.decode())
    assert response.status_code == 200
    assert response_json == {
        "message": f"Successfully stopped analysis for container {mock_ids_container.name}"
    }


@patch("app.routers.ids.push_alerts_to_loki")
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_network(
    push_alerts_mock, db_session_fixture: DatabaseSessionFixture, mock_background_tasks
):
    db_session = await db_session_fixture.get_db_session()
    ...
    response = await receive_alerts_from_ids(
        alert_data=alert_data, db=db_session, background_tasks=mock_background_tasks
    )
    response_json = json.loads(response.body.decode())
    assert response.status_code == 200
    assert response_json == {
        "content": "Successfully pushed alerts and metrics to Loki"
    }
...
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_static(
    calculate_and_push_mock,
    push_alerts_mock,
    db_session_fixture: DatabaseSessionFixture,
    ...
    alert_data: AlertData = AlertData(
        ...
        stop_time="01-01-2025 12:01:00.000000",
    )
    response = await receive_alerts_from_ids(
        alert_data=alert_data, db=db_session, background_tasks=mock_background_tasks
    )
...
@pytest.mark.asyncio
async def test_start_network_analysis_with_container_in_ensemble(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    mock_ids_container = await db_session_fixture.get_ids_container_model()
    ...
    print(response_json)
    assert response_json == {
        "error": f"container with id {mock_ids_container.id} is part of an ensemble. Hence, no individual analysis is possible"
    }


@pytest.mark.asyncio
async def test_start_static_analysis_with_container_in_ensemble(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    mock_ids_container = await db_session_fixture.get_ids_container_model()
    ...
    response = await start_network_container_analysis(
        network_analysis_data=static_analysis_mock_data, db=db_session
    )
    assert response.status_code == 500
    response_json = json.loads(response.body.decode())
    print(response_json)
