...
@pytest.mark.asyncio
async def test_setup_ensembles_successfull(
    add_container_mock, db_session_fixture: DatabaseSessionFixture
):
    db_session = await db_session_fixture.get_db_session()
    ensemble_data: EnsembleCreate = EnsembleCreate(
        name="ensemble1", description="ensemble-test", technique=0, container_ids=[0, 1]
    )
    ...
    expected_content = [
        {
            "content": "successfully added container 0 to ensemble None",
            "status_code": 200,
        },
        {
            "content": "successfully added container 1 to ensemble None",
            "status_code": 200,
        },
    ]
...
@patch(
    "app.routers.ensemble.deregister_container_from_ensemble", new_callable=AsyncMock
)
@pytest.mark.asyncio
async def test_remove_ensemble_succesful(
    deregister_mock, db_session_fixture: DatabaseSessionFixture
):
    db_session = await db_session_fixture.get_db_session()
    ensemble_id = 1
    ...
    assert response_json == {
        "content": [
            {
                "content": "message successfully removed container 1 from ensemble 1",
                "status_code": 200,
            }
        ]
    }
...
@pytest.mark.asyncio
async def test_remove_ensemble_failiure(
    deregister_mock, db_session_fixture: DatabaseSessionFixture
):
    db_session = await db_session_fixture.get_db_session()
    ensemble_id = 1
    ...
    assert response_json == {
        "content": [
            {
                "content": " Did not remove container 1 from ensemble 1 successfully",
                "status_code": 500,
            }
        ]
    }
...
async def test_start_static_ensemble_analysis_successful(
    ...
    response = await start_static_ensemble_analysis(
        static_analysis_data=static_analysis_data, db=db_session
    )
    response_json = json.loads(response.body.decode())
    assert response.status_code == 200
    assert response_json == {
        "content": [{"content": '{"message": "success"}', "status_code": 200}]
    }


@pytest.mark.asyncio
async def test_start_static_ensemble_analysis_failiure(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    static_analysis_data: StaticAnalysisData = StaticAnalysisData(
    ...
    assert response_json == {
        "content": [{"content": '{"message": "failed"}', "status_code": 500}]
    }


@pytest.mark.asyncio
async def test_start_static_ensemble_analysis_not_idle(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    static_analysis_data: StaticAnalysisData = StaticAnalysisData(
    ...
    assert response_json == {
        "error": f"container with id {mock_ids_container.id} is not Idle!, aborting"
    }


@pytest.mark.asyncio
async def test_start_static_ensemble_analysis_not_available(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    static_analysis_data: StaticAnalysisData = StaticAnalysisData(
    ...
    assert response.status_code == 500
    assert response_json == {
        "error": f"container with id {mock_ids_container.id} is not available! Check if it should be deleted"
    }
...
async def test_stop_analysis(db_session_fixture: DatabaseSessionFixture):
    ...
    stop_data: stop_analysisData = stop_analysisData(ensemble_id=1)
    ...
    assert response_json == {
        "content": [
            {
                "content": f"Successfully stopped analysis for container {mock_ids.id} and ensemble {mock_ensemble.id}",
                "status_code": 200,
            }
        ]
    }
...
async def test_stop_analysis_unsuccessful(db_session_fixture: DatabaseSessionFixture):
    ...
    assert response_json == {
        "content": [
            {
                "content": f"Could not stop analysis for container {mock_ids.id} and ensemble {mock_ensemble.id}",
                "status_code": 500,
            }
        ]
    }
...
@pytest.mark.asyncio
async def test_start_network_ensemble_analysis_successful(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    network_analysis_data: NetworkAnalysisData = NetworkAnalysisData(ensemble_id=1)
    ...
    response = await start_network_ensemble_analysis(
...
@pytest.mark.asyncio
async def test_start_network_ensemble_analysis_failiure(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    network_analysis_data: NetworkAnalysisData = NetworkAnalysisData(ensemble_id=1)
    ...
    assert response.status_code == 200
    assert response_json == {
        "content": [{"content": '{"message": "failiure"}', "status_code": 500}]
    }


@pytest.mark.asyncio
async def test_start_network_ensemble_analysis_unavailable(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    network_analysis_data: NetworkAnalysisData = NetworkAnalysisData(ensemble_id=1)
    ...
    assert response.status_code == 500
    assert response_json == {
        "error": "container with id 1 is not available! Check if it should be deleted"
    }


@pytest.mark.asyncio
async def test_start_network_ensemble_analysis_not_idle(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    network_analysis_data: NetworkAnalysisData = NetworkAnalysisData(ensemble_id=1)
...
async def test_finished_ensemble_analysis(db_session_fixture: DatabaseSessionFixture):
    ...
    finished_data: AnalysisFinishedData = AnalysisFinishedData(
        container_id=1, ensemble_id=1
    )

    response = await finished_ensemble_analysis(
        analysisFinishedData=finished_data, db=db_session
    )
    response_json = json.loads(response.body.decode())
    print(response_json)
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_unsuccessful_loki_push(
    bg_tasks, push_to_loki_mock, db_session_fixture: DatabaseSessionFixture
):
    db_session = await db_session_fixture.get_db_session()
    ...
    response = await receive_alerts_from_ids_for_ensemble(
        alert_data=alert_data, db=db_session, backgroundtasks=bg_tasks
    )
    response_json = json.loads(response.body.decode())
    print(response_json)
    assert response.status_code == 500
...
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_network_analysis_last_container(
    cleanup_mock,
    bg_tasks,
    push_to_loki_mock,
    get_alerts_mock,
    ...
    assert response.status_code == 200
    assert response_json == {
        "content": f"Successfully pushed alerts for ensemble {mock_ensemble.name}"
    }
...
@patch("app.routers.ensemble.BackgroundTasks")
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_network_analysis_not_last_container(
    bg_tasks,
    push_to_loki_mock,
    last_container_sending_mock,
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    alert_data: AlertData = AlertData(
    ...
    assert response.status_code == 200
    assert response_json == {
        "content": f"Successfully pushed alerts for container {mock_container.name}"
    }
...
@patch("app.routers.ensemble.BackgroundTasks")
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_static_analysis_not_last_container(
    bg_tasks,
    push_to_loki_mock,
    last_container_sending_mock,
    db_session_fixture: DatabaseSessionFixture,
...
@pytest.mark.asyncio
async def test_receive_alerts_from_ids_static_analysis_last_container(
    len_mock,
    bg_tasks,
    push_to_loki_mock,
    get_alerts_mock,
