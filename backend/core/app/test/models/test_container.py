...
def mock_ids_container():
    mock_host_system = DockerHostSystem(
        id=1, name="localhost", host="localhost", docker_port=2375
    )
    ids_container = IdsSystem(
        id=1,
        name="Test IDS",
        port=8080,
...
@pytest.mark.asyncio
async def test_is_busy(mock_ids_container: IdsSystem):
    mock_ids_container.status = STATUS.ACTIVE.value
    assert await mock_ids_container.is_busy() is True


@pytest.mark.asyncio
async def test_is_not_busy(mock_ids_container: IdsSystem):
...
def test_get_container_http_url_proper_host(mock_ids_container: IdsSystem):
    ...
    assert (
        docker_host
        == f"http://{mock_ids_container.host_system.host}:{mock_ids_container.port}"
    )
...
async def test_update_container_existing(db_session_fixture: DatabaseSessionFixture):
    ...
    container_update = IdsContainerUpdate(
        id=mock_container.id, configuration_id=2, ruleset_id=3, description="123-Test"
    )
...
@pytest.mark.asyncio
async def test_update_container_not_existing(
    db_session_fixture: DatabaseSessionFixture,
):
    db_session = await db_session_fixture.get_db_session()
    container_update = IdsContainerUpdate(
        id=999, configuration_id=2, ruleset_id=3, description="123-Test"
    )
...
async def test_update_container_no_changes(db_session_fixture: DatabaseSessionFixture):
    ...
    container_update = IdsContainerUpdate(
        ...
        configuration_id=mock_container.configuration_id,
        ruleset_id=mock_container.ruleset_id,
        description=mock_container.description,
    )
    await update_container(db=db_session, container=container_update)
