...
async def test_remove_container(deregister_mock, mock_ensemble: Ensemble, db_session_fixture: DatabaseSessionFixture):
    ...
    response = await mock_ensemble.remove_container(container_id=mock_container.id, db=db_session)
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_get_assigned_containers(mock_ensemble: Ensemble,db_session_fixture:DatabaseSessionFixture):
    db_session = await db_session_fixture.get_db_session()
...
async def test_start_static_analysis(mock_ensemble:Ensemble, db_session_fixture:DatabaseSessionFixture):
    ...
    responses = await mock_ensemble.start_static_analysis(dataset_id=mock_dataset.id, db=db_session)
...
async def test_start_network_analysis(mock_ensemble:Ensemble, db_session_fixture:DatabaseSessionFixture):
    ...
    responses = await mock_ensemble.start_network_analysis(network_analysis_data=network_analysis_data, db=db_session)
    assert responses[0].status_code == 200
...
async def test_container_is_last_one_running_in_ensemble_with_one_container(mock_ensemble: Ensemble, db_session_fixture: DatabaseSessionFixture):
    db_session = await db_session_fixture.get_db_session()
    mock_container = await db_session_fixture.get_ids_container_model()
    assert await mock_ensemble.container_is_last_one_running(container=mock_container, db=db_session) is True
