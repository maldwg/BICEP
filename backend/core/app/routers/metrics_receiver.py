...
async def receive_metrics(metrics: MetricPushRequest):
    ...
    try:
        ...
        prometheus_pushgateway = os.environ.get(
            "PROMETHEUS_PUSH_GATEWAY_URL", "prometheus-push-gateway:9091"
        )
        if not prometheus_pushgateway.startswith(
            "http://"
        ) and not prometheus_pushgateway.startswith("https://"):
...
"""
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code in [200, 202]:
                LOGGER.debug(
                    f"Successfully pushed metrics for container {metrics.container_id}"
                )
                return JSONResponse({"status": "success"}, status_code=200)
            else:
                LOGGER.warning(f"Failed to push to Prometheus: {response.status_code}")
                return JSONResponse(
                    {"status": "failed", "error": "Prometheus push failed"},
                    status_code=500,
                )
