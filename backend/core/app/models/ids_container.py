...
class IdsContainer(Base):
    ...
    async def setup(self):
        ...
        async with get_db_session_context() as db:
            ids_tool = await get_ids_by_id(self.ids_tool_id)
            self.name = f"{ids_tool.name}-{self.port}"
            config = await get_config_by_id(self.configuration_id)
            rulseset = None
            if ids_tool.requires_ruleset:
                rulseset = await get_config_by_id(self.ruleset_id)
    ...
    async def update_config(self, config_id):
    ...
    async def update_ruleset(self, ruleset_id):
        from  .configuration import Configuration
        async with get_db_session_context() as db:
    ...
    async def start_metric_collection(self, stream_metric_tasks):
        async with get_db_session_context() as db:
    ...
    async def stop_metric_collection(self, stream_metric_tasks):
        async with get_db_session_context() as db:
...
async def remove_container_by_id(id: int):
    async with get_db_session_context() as db:
...
async def update_container(container: IdsContainerUpdate):
    async with get_db_session_context() as db:
