...
def get_host_by_id(id: int, db: Session):
    return db.query(HostSystem).filter(HostSystem.id == id).first()

def get_all_hosts(db: Session):
    return db.query(HostSystem).all()


def add_host_system(host: HostSystem ,db:Session):
    db.add(host)
    db.commit()
    db.refresh(host)

def remove_host(host_id: int ,db:Session):
    host: HostSystem = get_host_by_id(host_id, db)
    db.delete(host)
    db.commit()